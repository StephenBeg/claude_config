...
- **`/plan`** — besoin large à découper. Analyse, plan + DAG, tickets JIRA parallélisables, puis (sur GO) supervise un `/dev` par ticket jusqu'au drainage. Trop gros → sous-plans par domaine. Détail complet dans la commande `/plan`.
- **`/dev`** — un ticket JIRA fourni à implémenter de bout en bout (worktree → tests → MR → suivi pipeline → statuts). Détail complet dans la commande `/dev`. Ne jamais merger la MR soi-même.
...
Avant de lancer un process long (boucle `until` background, suivi pipeline, `await`/spawn d'orchestrateur, smoke-run), empêcher la mise en veille auto du Mac. **Toujours vérifier d'abord qu'un `caffeinate` ne tourne pas déjà** (ne jamais le lancer deux fois) :
```
...
|---|---|
| Règles communes `/dev` `/plan` `/hotfix` (questions à choix, escalade archi, accès JIRA, préfixes CMUX, vérif sources, vérif & boucles, smoke-run, /end, travail découvert, livrable) | `malt-workflow-commons` (invoqué en 1er par chaque workflow) |
| Suivi de pipeline GitLab CI (verte, parent-child, Sonar, conflits rebase, boucle d'attente) | `malt-pipeline-followup` |
| Déléguer à des sous-agents, dimensionner un model, gérer le contexte, transiter du /tmp | `malt-orchestration` |
