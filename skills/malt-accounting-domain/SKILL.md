...
- **Puis un transport switch** (BILL-2923/2928/2937) : port neutre `NetsuiteRecordWritePort` + FF global `accounting-netsuite-rest-write` (**OFF = SOAP, ON = REST**). Cinq selectors lisent ce seul flag ; les beans SOAP sont gatés `@ConditionalOnProperty("netsuite.soap.account-id")`, fallback REST si le bean SOAP absent. accounting peut donc écrire via l'un OU l'autre transport, mais les deux codebases restent séparées. Voir [[reference_accounting_neutral_write_port_transport_flag]], [[reference_accounting_ff_transport_switch_e2e]].
